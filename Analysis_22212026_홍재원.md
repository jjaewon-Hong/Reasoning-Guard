...
본 시스템의 사용자 인터페이스는 전장의 다채널 카메라 입력을 실시간으로 종합하는 **거시적 관점의 '종합 전술 상황도(COP)'**와, 단일 표적에 대한 AI 추론 결과를 시각화하는 **미시적 관점의 '사격 통제 제원(FCS) 인터페이스'** 두 가지 패널로 분리하여 구성된다.

특히, 추론 엔진이 도출한 확률(Confidence Score)에 따라 **SAFE(평시) ➔ CAUTION(경고) ➔ DANGER(교전)** 3단계로 이어지는 **단계별 대응(Tiered Response) 로직**을 UI에 직관적으로 반영하였다. 이를 통해 관제병의 전술적 판단을 보조하고 오발사를 원천적으로 차단한다.

<br>

### 1) [SAFE 상태] 평시 모니터링 및 대기 단계

[그림 5]와 [그림 6]은 전장 내 특별한 위협이 없거나, 식별된 객체의 위협 확률이 임계치(40%) 미만일 때 출력되는 평시 상태의 UI이다.

![그림5](./assets/그림5.png)
*<center>[그림 5] SAFE 상태: 종합 전술 상황도 (COP) 플랫폼</center>*

* 다채널 카메라 배열을 통해 전 구역을 모니터링하며, 전반적인 시스템 상태가 'SAFE'임을 녹색 UI로 나타낸다. 화면 우측 하단의 탐지 객체 카운트는 위협이 없음을 표시한다.
...
* 레이더에 미식별 객체가 포착되어 ResNet-18 기반의 특징 추출이 진행되었으나, 적대적 미사일일 확률(Confidence)이 35.0%로 낮게 측정된 상황이다. 오발사 방지(Fail-safe) 메커니즘이 즉각 작동하여 최종 판정을 **'HOLD(대기)'**로 도출하며, 녹색 바운딩 박스를 통해 현재 타격 대상이 아님을 관제병에게 시각적으로 알린다.

<br>

### 2) [CAUTION 상태] 위협 감지 및 교전 결심 단계

[그림 7]과 [그림 8]은 식별된 객체의 위협 확률이 경계 임계치(40% ~ 90%)에 도달했을 때 전환되는 위협 감지 상황의 UI이다.

![그림7](./assets/그림7.png)
*<center>[그림 7] CAUTION 상태: 종합 전술 상황도 (COP) 플랫폼</center>*

* 특정 섹터의 카메라(CAM 03, 07 구역 등)에서 잠재적 위협이 감지되어 해당 구역이 붉은색으로 강조(LOCK)된다. 전역 상태는 황색의 'CAUTION'으로 격상되며, 미사일 추정 객체의 카운트가 급증하여 지휘통제실의 주의를 환기한다.
...
* 추론 결과 타겟이 미사일일 확률이 50.4%로 산출된 상황이다. 노란색 바운딩 박스로 표적을 정밀 추적하며, 시스템은 교전 권고 상태인 **'SHOOT(사격)'** 판정을 내린다. 이때 좌측 하단의 외부 사격 통제 제원(TGT_POS, AZI/ELEV 등) 인터페이스와 우측의 아군 가용 무장(Current Ordnance) 현황이 연동되어, 관제병이 즉각적인 교전 승인 의사결정을 내릴 수 있도록 데이터를 지원한다.

<br>

### 3) [DANGER 상태] 위협 확정 및 교전 실행 단계

[그림 9]와 [그림 10]은 객체의 위협 확률이 자동 교전 임계치(90% 이상)를 초과하여, 찰나의 지연 없이 즉각적인 타격이 요구되는 상황의 UI이다.

![그림9](./assets/그림9.png)
*<center>[그림 9] DANGER 상태: 종합 전술 상황도 (COP) 플랫폼</center>*

* 고위협 표적의 쇄도가 시스템적으로 확정됨에 따라 화면 전반에 적색의 'DANGER' 경보가 점등되며, 방공망 전체가 최고 수준의 비상 상태로 전환됨을 알린다.

<br>

![그림10](./assets/그림10.png)
*<center>[그림 10] DANGER 상태: 전술 객체 정밀 식별 및 사격 통제 제원(FCS) 인터페이스</center>*

* 객체가 미사일일 확률이 92.4%로 확정된 긴박한 상황이다. 표적을 적색 바운딩 박스로 락온(Lock-on)함과 동시에 최종 판정이 **'FIRED(발사)'**로 전환된다. 이는 시계열적으로 매우 짧은 대응 시간 내에 인간의 인지 및 승인 지연을 극복하기 위해, 시스템이 산출된 사격 제원을 바탕으로 무장 시스템에 즉각적인 자동 타격 명령을 수행했음을 의미한다.

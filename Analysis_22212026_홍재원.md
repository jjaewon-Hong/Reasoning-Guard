...
본 프로젝트는 일반적인 사용자 상호작용(Use Case) 위주의 구현보다, 오발사 방지 및 실시간 추론을 위한 시스템적 아키텍처 설계 자체에 핵심 가치를 둠. 이에 따라 본 도메인 분석 단계에서는 단순 기능 중심의 접근을 지양하고, Reasoning Guard 엔진을 실질적으로 구동하는 핵심 논리적 클래스들을 학습(PyTorch) 및 운용(C++) 도메인으로 분리하여 식별함.

...
| **Success Post Condition** | 유효한 데이터 검증을 거쳐 DANGER, SAFE, CAUTION 상태중 하나의 상태를 반환함 |

| MAIN | SUCCESS SCENARIO |
| :--- | :--- |
| **Step** | **Action** |
| 1 | 관제병이 인터페이스의 **'+1 Hour Simulation'** 버튼을 클릭함 |
| 2 | 시스템이 내부 저장소(docs)의 12종 상황 이미지 중 하나를 무작위로 호출하여 로드함 |
| 3 | 시스템이 입력 데이터의 무결성을 확인하고 전처리를 수행함 |
| 4 | 시스템이 4개 클래스 독립 확률 분석을 통해 최종 판정 결과를 출력함 |
...
| :--- | :--- |
| **Use Case Name** | **Identify Aerial Target** |
| **Summary** | 관제병이 **'+1 Hour Simulation'** 버튼을 클릭하여 시나리오 상의 표적을 호출하고, AI를 통해 정체 판독 및 대응을 결정함 |
| **Scope** | Reasoning Guard System (FailSafeValidator) |
| **Level** | Sub-function level |
| **Primary Actor** | User (Operator / 관제병) |
| **Preconditions** | `+1 Hour Simulation`을 통해 데이터가 메모리에 로드됨 |
...
| GENERAL CHARACTERISTICS | 결정 확인 (4-Class 독립 확률 분석 포함) |
| :--- | :--- |
| **Use Case Name** | **Confirm Decision** |
| **Summary** | ONNX 추론 엔진의 연산 결과를 바탕으로 4개 클래스의 독립 확률을 분석하고 최종 교전 수칙을 UI에 시각화함 |
...
| :--- | :--- |
| **Step** | **Action** |
| 1 | C++ 엔진이 ONNX 모델을 구동하여 **전투기, 드론, 로켓, 비위협(etc)** 각 클래스별 독립 확률(Sigmoid)을 산출함 |
| 2 | 시스템이 **최고 위협 확률(Max Threat)**과 **비위협 확률(etc)**을 복합적으로 비교하여 전술 등급을 판정함 |
| 3 | 판정 결과에 따라 SAFE, CAUTION, DANGER 상태를 UI 화면에 즉각 렌더링함 |

| EXTENSION SCENARIOS | |
| :--- | :--- |
| **Step** | **Branching Action** |
| 2 | **2a. Determine Hold (Extend):** 위협 확률이 경합 상태(40~90%)이거나 판정이 불확실할 경우 <br> 2a1. 'CAUTION' 상태를 발령하고 관제병의 수동 승인을 대기함 |
...
| **OperationalResponse** | 운용<br>도메인 | 식별된 확률 정보([F, D, R] Max vs ETC)를 통합하여 SAFE, CAUTION, DANGER의 3단계 전술 지침을 도출함 |
...
[그림 5]와 [그림 6]은 **최고 위협 확률(Threat)이 40.0% 미만**인 상황.
...
[그림 7]과 [그림 8]은 식별된 객체의 **최고 위협 확률이 경계 임계치(40% ~ 90%)** 혹은 **최고 위협 확률(Threat)이 90% 이상이지만 비위협 확률도 50% 이상**인 경우 전환되는 위협 감지 상황의 UI임.

![그림7](./assets/_2_analysis/tv2.png)
*<center>[그림 7] CAUTION 상태: 종합 전술 상황도 (COP) 플랫폼</center>*

* 특정 섹터의 카메라(CAM 03, 07 구역 등)에서 잠재적 위협이 감지되어 해당 구역을 붉은색으로 강조(LOCK)함. 전역 상태는 황색의 'CAUTION'으로 격상되며, **Manual Override**를 요청함. 관제병이 최종 승인을 인가해야만 실제 타격 데이터가 인계되는 핵심 인간 개입 구간임.
...
* **최고 위협 확률이 90.0%를 초과**하고, 동시에 **비위협 확률이 50.0% 미만**인 위협 대상으로 확정된 상황. 표적을 적색 바운딩 박스로 락온(Lock-on)함과 동시에 최종 판정이 **'FIRED(발사)'** 로 전환됨. 이는 시계열적으로 매우 짧은 대응 시간 내에 인간의 인지 및 승인 지연을 극복하기 위해, 식별 엔진이 산출된 데이터를 바탕으로 무장 시스템에 자동 타격 명령을 하달했음을 나타냄.

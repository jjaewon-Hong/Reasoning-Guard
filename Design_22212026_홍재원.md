...
* **설명**: 카메라나 센서로부터 입력받은 원본 이미지 데이터를 AI 모델이 이해할 수 있는 정규화된 텐서(Tensor) 형태로 변환합니다. 학습 환경(PyTorch)의 파이프라인을 C++에서 동일하게 재현함.
* **관련 파일**: `preprocess.h`
* **Attributes (속성)**
...
* **설명**: ONNX Runtime(ORT) API를 제어하여 전처리된 데이터를 모델에 통과시키고 결과값을 받아오는 엔진 컨트롤러임.
...
* **설명**: 단순한 AI의 수학적 연산 결과(점수)를 실제 전술적인 의미로 해석하는 시스템의 "두뇌" 역할을 함.
...
* **설명**: C++로 작성된 고성능 전술 엔진(TacticalEngine 및 Preprocess)을 브라우저에서 실행할 수 있도록 WebAssembly로 래핑(Wrapping)함.
